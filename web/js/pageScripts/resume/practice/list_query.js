        ...
        $('body').on('click', ".opt li a", function() {
          ...
          /*  if(index==0){//查看详情
                ...
                if(!($this.hasClass("no-editable"))){
                    var reqUrl = webBasePath+'/employments/'+id;
        ...
        jQuery.ajax({
            ...
            success: function (result) {
                if (result.success) {
                    ...
                    if (result.employments != null && result.employments.length > 0) {
                        var data = result.employments,clz;
                        var _html = new Array();
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            _html.push('<tr>');
                            _html.push('<td>' + obj.positionName + '</td>');
                            _html.push('<td style="width: 30px;">' + obj.number + '</td>');
                            _html.push('<td style="width: 150px;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;" title="'+obj.positionDesc+'">' + obj.positionDesc + '</td>');
                            _html.push('<td style="width: 100px;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;" title="'+obj.address+'">' + obj.address + '</td>');
                            _html.push('<td>' + (obj.beginTime).replace(/(\d{4}).(\d{1,2}).(\d{1,2}).+/mg, '$1-$2-$3') + '</td>');
                            _html.push('<td>' + (obj.endTime).replace(/(\d{4}).(\d{1,2}).(\d{1,2}).+/mg, '$1-$2-$3') + '</td>');
                            _html.push('<td>' + obj.browerVolume + '</td>');
                            _html.push('<td>' + obj.applyVolume + '</td>');
