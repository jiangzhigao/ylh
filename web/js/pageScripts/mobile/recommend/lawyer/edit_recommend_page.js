jQuery(function(){
    ...
    function _bind() {
        ...
        $('#btnSave').on('click', function () {
            var $this = $(this);
            var id = $("#dataId").val();
            _ajax($this, '保存',webBasePath+'/banners/'+id);
        });
        //返回
    ...
    function _initData (id) {
        ...
        jQuery.ajax({
            dataType: "json",
            url: webBasePath + '/banners/'+id,
            data: queryParams,
            type: "GET",
            success: function (result) {
                if (result.success) {
                    var data = result.banner;
                    if(data){
                        $("#dataId").val(data.id);
                        $("#title").val(data.title);
                        $("#desc").val(data.description);
                        if(data.pic){
                            $("#coverUrl").val(data.pic);
                            $("#coverImage").attr("src",data.pic);
                            $("#imgBox").show();
                        }
                        $("input[name='linkType'][value='"+data.linkType+"']").attr("checked",true);
                        $("#linkModule").val(data.linkModule);
                        $("#jumpLink").val(data.jumpLink);
