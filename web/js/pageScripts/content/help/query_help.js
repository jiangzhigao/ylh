jQuery(function(){
        ...
        $('body').on('click', ".opt li a", function() {
            ...
            if($this.parent().index()==0){
                ...
                if(!($this.hasClass("no-editable"))){
                    var reqUrl = webBasePath+'/agreementHelps/'+id;
    ...
    function _initData () {
        ...
        jQuery.ajax({
            dataType: "json",
            url: webBasePath + '/agreementHelps',
            data: queryParams,
            type: "GET",
            success: function (result) {
                if (result.success) {
                    ...
                    if (result.agreementHelps != null && result.agreementHelps.length > 0) {
                        var _html = new Array();
                        var data = result.agreementHelps;
