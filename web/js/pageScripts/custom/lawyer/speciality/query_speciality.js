jQuery(function(){
    ...
    function _optionsHtml(id){
        ...
        _operHtml.push('<li style="border-bottom: 1px dashed #CCC;"><a bz-url="/view/customercenter/lawyermanagement/speciality/editspeciality_.jsp" bid="'+id+'" target="ylxmain">编辑</a></li>');
            ...
            success: function (result) {
                if (result.success) {
                    ...
                    if (result.professionalFields != null && result.professionalFields.length > 0) {
                        ...
                        options.totalPages = _sumTotalPages(result.professionalFields.length);
                        ...
                        $pageTotalRecord.html('<div class="dataTables_info" role="status" aria-live="polite"> 共'
                             + result.professionalFields.length + '条记录，当前为第 ' + options.currentPage + ' 页');
                    } else {
                        $('#batchDeleteDiv').hide();
